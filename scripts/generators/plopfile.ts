      ...
      return [
        {
          ...
          content: [
            ...
            '',
            `### 菜单注册`,
            `  在 demo/frontend/src/menu-trees.ts 的对应 AppType 菜单树中添加页面节点（内联组件）：`,
            `  { path: '${pageName}', name: '${answers.name}', icon: '${icon}', component: () => import('./views/${moduleGroup}/${pageName}/Index.vue'), permissions: [${permValues?.map(v => `'${v}'`).join(', ') || ''}] }`,
            '',
            `### 列表页（Index.vue）`,
