      ...
      return [
        {
          ...
          content: [
            ...
            '',
            `### 文件结构`,
            `  demo/frontend/src/views/${moduleGroup}/${pageName}/`,
            `  ├── Index.vue                  ← 列表页（MfwPageWrapper + MfwListPage）`,
            `  ├── ${pascalName}Form.vue      ← 表单组件（MfwFormCard + MfwPopup）`,
            '',
            `### 菜单注册`,
            `  在 demo/shared/src/menu-trees.ts 的对应 AppType 菜单树中添加页面节点：`,
            `  { path: '${moduleGroup}/${pageName}', name: '${answers.name}', icon: '${icon}', permissions: [${permValues?.map(v => `'${v}'`).join(', ') || ''}] }`,
            '',
            `  同时在 demo/frontend/src/router.ts 的 componentMap 中注册组件：`,
            `  '${moduleGroup}/${pageName}': () => import('./views/${moduleGroup}/${pageName}/Index.vue'),`,
            '',
            `### 列表页（Index.vue）`,
            ...
            '',
            `### 页面配置（menu-trees.ts）`,
            `  - 菜单树节点: { path, name, icon, auth: true, permissions }`,
      ...
      return [
        {
          ...
          content: [
            ...
            '',
            `### 文件结构`,
            `  packages/base/src/frontend/components/${category}/${componentName}/`,
            `  ├── ${entryFile}         ← 入口文件`,
