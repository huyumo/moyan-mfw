 ...
 * 代码质量门禁 Hook
 *
 * 目的：确保 AI 编写的代码通过类型检查、Lint 检查和自测
 *
 * 检查项：
 * 1. TypeScript 类型检查（无 any，类型完整）
...
/**
 * 动态检测子项目
 * 扫描项目根目录下包含 package.json 和 src/ 目录的子目录
 */
function detectSubProjects(projectRoot: string): SubProject[] {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      // 检查是否是有效子项目：有 package.json 和 src 目录
...
function checkQAApproval(projectRoot: string): { approved: boolean; status?: string; error?: string } {
  ...
  if (!fs.existsSync(taskFilePath)) {
    return { approved: false, error: '未找到 TASK.md 文件' };
  }
  ...
  if (!frontMatterMatch) {
    return { approved: false, error: 'TASK.md 格式错误：缺少 Front Matter' };
  }
  ...
  // 如果是 completed 状态，也允许（QA 提交后的状态）
  if (status === 'completed') {
    return { approved: true, status };
  }
  ...
  return {
    ...
    error: `任务状态为 "${status}"，必须是 "qa_approved" 或 "completed" 才能提交`
  };
}
...
export async function run(args: string[]): Promise<HookResult> {
  ...
  const harnessOutputDir = path.join(projectRoot, '.claude', 'harness', 'output');

  // 检查 1: 查找最近修改的文件（通过输出目录的记录）
  ...
  try {
    ...
    // 如果所有 typecheck 命令都失败，尝试直接使用 tsc（从项目 node_modules）
    if (!typeCheckPassed) {
      try {
    ...
    // 如果还是失败，尝试使用 vue-tsc（针对 Vue 项目）
    if (!typeCheckPassed && !typeCheckError.includes('This is not the tsc command')) {
      try {
        const subProjects = detectSubProjects(projectRoot);
  ...
  // 检查 3: 检查自测记录（支持子项目目录）- 增强版
  // 不仅检查文件存在，还要验证测试是否实际执行
  // 动态检测子项目并构建自测报告路径列表
  const subProjects = detectSubProjects(projectRoot);
  ...
  for (const sub of subProjects) {
    selfTestFiles.push(
      path.join(sub.path, '.claude', 'harness', 'output', 'self-test-report.md')
    );
  }
  ...
  if (!hasSelfTest) {
    ...
    for (const testResult of selfTestResults.filter(r => r.exists)) {
      ...
      // 检查 3: 是否有"开发中"、"未完成"、"TODO"等标记
      const pendingPatterns = ['开发中', '未完成', '待实现', 'TODO', 'FIXME', '待执行'];
  ...
  // 检查 4: 检查自我反思记录
  const selfReviewFile = path.join(harnessOutputDir, 'self-review.md');
  ...
  if (e2eTestDir) {
    ...
    if (!hasE2eEvidence) {
      ...
      result.errors.push('   请运行 E2E 测试并生成报告（playwright-report 或 test-results 目录）');
      result.passed = false;
    }
  }

  // 检查 6: 功能完成度检查 - 扫描代码中的"开发中"等标记
  // 动态构建源代码目录列表
  ...
  const incompletePatterns = [
    ...
    { pattern: /未完成/gi, label: '未完成' },
    { pattern: /待实现/gi, label: '待实现' },
  ...
  // 检查 5: 检查代码中是否有明显的 TODO/FIXME 标记（新添加的）
  if (modifiedFiles.length > 0) {
      ...
      if (fs.existsSync(file)) {
        try {
          ...
          if (/TODO(?!.*完成)/i.test(content) || /FIXME/i.test(content)) {
            ...
            result.warnings.push(`⚠️ ${path.relative(projectRoot, file)} 中包含 TODO/FIXME`);
          }
        } catch (e) {
          // 忽略
        }
    ...
    if (hasPendingTodos) {
      result.warnings.push('// TODO-TASK-2026-04-11-001: 建议完成所有 TODO 标记的工作后再提交');
    }
  }

  // 检查 6: QA 审批状态检查（新增 - 开发测试循环强制要求）
  ...
  if (!typeCheckPassed && typeCheckError) {
    result.passed = false;
    result.message = '【阻塞】代码质量检查失败 - 类型检查未通过';
    return result;
  }
  ...
  if (!qaApproved) {
    result.passed = false;
    result.message = '【阻塞】代码质量检查失败 - QA 未审批通过';
    return result;
  }

  result.data = {
    typeCheckPassed: typeCheckPassed || result.warnings.some(w => w.includes('类型检查执行异常')),
  ...
  // 输出日志
  const logFile = path.join(projectRoot, '.claude', 'harness', 'output', 'code-quality-gate.log');
