...
sequenceDiagram
    autonumber
    participant U as 用户
    participant P as 前端页面<br/>(RoleListPage)
    ...
    rect rgb(240, 250, 230)
        ...
        A-->>P: 权限池数据
        note right of P: 后续权限选择器<br/>仅展示权限池中的节点
    end
    ...
    rect rgb(255, 240, 240)
        ...
        P->>P: 收集所有勾选的权限编码
        P->>P: 去重 + 过滤空值
        P->>A: POST /sys/role/:code/permissions<br/>{permissionCodes: [...]}
        ...
        D-->>S: 删除成功
        S->>D: INSERT INTO sys_role_permission<br/>(roleId, permissionId) VALUES ...
        D-->>S: 插入成功
        S->>D: COMMIT
...
```mermaid
flowchart TD
    Start[开始权限分配] --> CollectCodes[收集勾选的权限编码]
...
| 类型 | 说明 | 权限分配 |
|------|------|----------|
| 内置角色 | 不绑定 appId，仅绑定 appTypeId，为应用类型全局角色 | 不允许在前端分配权限 |
| 应用级角色 | 必须绑定 appId，属于具体应用实例 | 可在应用实例详情页分配权限 |
