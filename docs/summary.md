# Moyan MFW 权限系统文档

## 📖 文档导航

### 入门指南

* [首页](README.md)
* [术语表](glossary.md)

---

## 🎯 核心概念模块

* [权限系统核心概念](core/permissions.md)
* [角色体系核心概念](core/roles.md)
* [系统架构核心概念](core/architecture.md)

---

## 💾 数据库设计

* [数据库实体设计](database/database-entities-design.md)
* [数据库 ER 关系图](database/database-er-diagram.md)

---

## 🔄 业务流程

* [权限分配流程](flows/permission-assignment.md)
* [权限池配置流程](flows/permission-pool-setup.md)
* [系统初始化流程](flows/system-initialization.md)
* [用户登录流程](flows/user-login-flow.md)
* [权限计算规则](flows/permission-calculation-rules.md)
* [开发者模式说明](flows/developer-mode.md)

---

## 📄 页面设计

* [应用类型管理页面](pages/app-type-management.md)
* [应用实例管理页面](pages/app-management.md)
* [角色管理页面](pages/role-management.md)
* [成员管理页面](pages/member-management.md)
* [权限管理页面](pages/permission-management.md)

---

## 🔌 API 接口文档

* [API 接口索引](api/api-index.md)
* [统一类型定义](api/types.md)
* [错误码定义](api/error-codes.md)

### 认证接口

* [认证接口](api/auth-api.md)

### 用户接口

* [用户接口](api/user-api.md)

### 应用类型接口

* [应用类型接口](api/app-type-api.md)

### 应用接口

* [应用接口](api/app-api.md)

### 角色接口

* [角色接口](api/role-api.md)

### 成员接口

* [成员接口](api/member-api.md)

### 权限接口

* [权限接口](api/permission-api.md)

### 审计日志

* [审计日志设计](api/audit-log-design.md)

---

## 🎨 UI 规范

* [异常状态 UI 规范](ui/ui-error-states.md)

---

## 📚 开发指南

* [典型应用场景](guides/use-cases.md)
* [错误处理流程](guides/error-handling.md)

---

## 📊 追踪文档

* [问题追踪](tracking/REVIEW-ISSUES.md)
* [变更日志](tracking/CHANGELOG.md)
* [文档评审报告](tracking/REVIEW-REPORT.md)
* [项目交付报告](tracking/PROJECT-DELIVERY-REPORT.md)
* [三团队会议纪要](tracking/THREE-TEAM-MEETING-MINUTES.md)
* [综合评审报告](tracking/COMPREHENSIVE-REVIEW-REPORT.md)
* [改进行动计划](tracking/IMPROVEMENT-ACTION-PLAN.md)
* [PM 建议书](tracking/PM-RECOMMENDATIONS.md)

---

## 📁 基础设施文档

* [基础设施详细设计索引](infrastructure-detailed-design-index.md)
* [基础设施详细设计](infrastructure-detailed-design.md)

---

*本文档由 Moyan MFW 团队维护 | 版本：2.0.0*
