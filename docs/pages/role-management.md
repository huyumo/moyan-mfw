...
1. **检查约束**：
   - 如果是拥有者角色 (`isOwner = 1`)，不允许删除
   - 检查是否有用户绑定了该角色（可选：有用户绑定时禁止删除或自动解绑）
...
**事务处理**：
- 以上操作应该在同一个事务中执行，保证数据一致性
- 如果任意一步失败，整个操作回滚

**业务规则**：
- 拥有者角色 (`isOwner = 1`) 不允许删除
- 内置角色可以删除（在应用类型管理页面），但需要谨慎操作，因为会影响所有绑定该角色的用户
- 删除应用级角色前，建议先检查是否有用户绑定了该角色，避免影响现有用户权限
