...
import { runSeeds } from "../../src/backend/src/database/seeds/index";
import {
  initPermissionValueCache,
  getPermissionValues,
} from "../../src/backend/src/common/constants/permissions";
import { config } from "dotenv";
import * as path from "path";

// Jest 执行顺序：globalSetup 先于 setupFiles，此处主动加载测试环境变量
config({
  path: path.join(__dirname, "../../src/backend/.env.test"),
  override: true,
});

// runSeeds 依赖 permissionValueCache（生产环境由 PermissionValueSyncService 在应用启动后填充）。
// 种子阶段缓存为空会导致 buildPerValue 抛"未知的权限名称"，此处按内置权限顺序预置缓存。
initPermissionValueCache(
  getPermissionValues().map((name, i) => ({
    name,
 ...
 * 清理所有 sys_* 表
 */
async function cleanupSysTables(
  connection: Connection,
  dbName: string,
): Promise<void> {
  const queryRunner = connection.createQueryRunner();
...
export default async function globalSetup() {
  ...
  try {
    ...
    const connection = await createConnection({
      ...
      logging: false,
      entities: [
        __dirname + "/../../src/backend/src/modules/sys/**/*.entity{.ts,.js}",
      ],
    } as any);
    ...
    if (isEmpty) {
      console.log(
        "[GlobalSetup] Database is empty, initializing with seed data...",
      );
      ...
      console.log(
        "[GlobalSetup] Database is not empty, cleaning and re-initializing...",
      );
    ...
    const systemAppType = await (
      global as any
    ).TEST_DB_CONNECTION.manager.findOne("sys_app_types", {
      where: { typeCode: "system" },
    });
    const superAdminRole = await (
      global as any
    ).TEST_DB_CONNECTION.manager.findOne("sys_roles", {
    ...
    console.log(
      `[GlobalSetup] Seed data IDs: adminUserId=${(global as any).SEED_DATA.adminUserId}`,
    );
  } catch (error) {
