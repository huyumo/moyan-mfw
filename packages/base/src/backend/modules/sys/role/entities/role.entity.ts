...
export class Role extends Base {
   ...
   */
  @Column({ type: 'char', length: 36, nullable: true, comment: '应用实例 ID - 关联的应用实例，为空时表示全局角色' })
  @Index()
  appId: string;
   ...
   */
  @Column({ type: 'char', length: 36, nullable: true, comment: '应用类型 ID - 关联的应用类型，为空时表示全局角色' })
  @Index()
  appTypeId: string;
   ...
   */
  @Column({ type: 'varchar', length: 64, comment: '角色名称 - 用于展示的角色别名' })
  roleName: string;
   ...
   */
  @Column({ type: 'varchar', length: 64, comment: '角色编码 - 角色的唯一标识，全局唯一' })
  roleCode: string;
   ...
   * @description 角色描述信息
   */
  @Column({ type: 'varchar', length: 255, nullable: true, comment: '角色描述信息' })
  roleDesc: string;
