...
async function columnExists(conn: any, table: string, column: string): Promise<boolean> {
  const [rows] = await conn.execute(
    ...
    [process.env.DB_NAME || 'moyan_mfw', table, column]
  );
  ...
  try {
    ...
    if (await columnExists(connection, 'sys_user_roles', 'appId')) {
      ...
      await connection.execute(
        `ALTER TABLE sys_user_roles ADD COLUMN appId CHAR(36) NULL COMMENT '应用实例 ID'`
      );
    ...
    `);
    process.stdout.write(`  ✓ 已更新 ${(updateResult as any).changedRows || 0} 条记录\n`);
    ...
    process.stdout.write(`  ✓ 已删除 ${(deleteResult as any).affectedRows || 0} 条记录\n`);
    ...
    const [colInfo] = await connection.execute(
      `SELECT IS_NULLABLE FROM information_schema.COLUMNS
       WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'sys_user_roles' AND COLUMN_NAME = 'appId'`,
      [process.env.DB_NAME || 'moyan_mfw']
    );
    if ((colInfo as any[])[0]?.IS_NULLABLE === 'NO') {
      process.stdout.write('  ✓ appId 已是 NOT NULL，跳过\n');
    } else {
      await connection.execute(
        `ALTER TABLE sys_user_roles MODIFY COLUMN appId CHAR(36) NOT NULL COMMENT '应用实例 ID - 角色分配所属的具体应用实例'`
      );
      process.stdout.write('  ✓ 完成\n');
    }
    ...
    // 检查新索引是否已存在
    const [newIdx] = await connection.execute(
      `SHOW INDEX FROM sys_user_roles WHERE Key_name = 'idx_unique_user_role_app'`
    );
    if ((newIdx as any[]).length > 0) {
      process.stdout.write('  ✓ 新唯一约束已存在，跳过\n');
    } else {
      // 删除旧的唯一索引（去重：SHOW INDEX 每列返回一行）
      const [indexes] = await connection.execute(
        `SHOW INDEX FROM sys_user_roles WHERE Non_unique = 0 AND Key_name != 'PRIMARY'`
      );
    ...
    const [appIdIndexes] = await connection.execute(
      `SHOW INDEX FROM sys_user_roles WHERE Key_name = 'idx_user_roles_appId'`
    );
    if ((appIdIndexes as any[]).length > 0) {
