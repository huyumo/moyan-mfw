...
exports.createExtensionBackendApp = createExtensionBackendApp;
/**
...
function validateManifest(manifest) {
    ...
    if (!manifest.routePrefix.startsWith('/ext/')) {
        throw new Error(`[Extension] routePrefix 必须 /ext/{ns} 格式，当前: "${manifest.routePrefix}"`);
    }
    ...
    for (const node of manifest.permCodeNodes) {
        ...
        if (!node.permCode.startsWith(`${ns}:`)) {
            throw new Error(`[Extension] permCode "${node.permCode}" 必须以命名空间 "${ns}:" 开头`);
        }
    }
    console.log(`[Extension] ✅ ${manifest.displayName} v${manifest.version} 清单校验通过`);
}
async function createExtensionBackendApp(options) {
    ...
    const result = await (0, create_base_backend_app_1.createBaseBackendApp)({
        name: options.name,
...
//# sourceMappingURL=create-extension-backend-app.js.map