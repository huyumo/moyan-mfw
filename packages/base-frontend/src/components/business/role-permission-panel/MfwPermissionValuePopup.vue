<!--
/**
 * @fileoverview 角色权限值配置弹窗
 * @description 用于配置角色权限中节点的 permissionValue
 * 可选值范围来自权限池中该节点的 permissionValue（子集约束）
 */
-->
<template>
...
import { ApiAppTypeGetPermissionPool } from '../../../apis/sys';
...
// 根据权限池中的 permissionValue 生成可选的权限选项（子集约束）
...
onMounted(async () => {
  // 1. 获取权限池配置，得到该节点在权限池中的 permissionValue
  try {
...
/**
 * 更新角色权限中的权限值
 * 返回修改后的 permissionValue，由主组件处理提交
 */
