...
if (!fs.existsSync(distFile)) {
  console.log(`${FAIL}  借贷记账: dist/index.mjs 缺失，请先运行 pnpm build`);
  process.exit(1);
}
...
if (!/MfwLedgerPage\b/.test(content)) {
  console.log(`  ${FAIL}  exports MfwLedgerPage`);
  console.log('       → 确保 src/index.ts 中正确导出 MfwLedgerPage');
  failed++;
} else {
  console.log(`  ${PASS}  exports MfwLedgerPage`);
}

console.log('');
if (failed > 0) {
  console.log(`${FAIL}  借贷记账: ${failed} 项检查未通过 — 阻塞发布`);
  process.exit(1);
} else {
  console.log(`${PASS}  借贷记账 dist 产物检查通过`);
}
