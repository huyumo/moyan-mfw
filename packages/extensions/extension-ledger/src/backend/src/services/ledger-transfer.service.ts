 ...
 * @description 制单（同步预占+入队）、审核、冲正、重推、取消；注入 storage/queue/notifier SPI
 */
...
import type { ILedgerStorage, ILedgerQueue, ILedgerNotifier, ILedgerFieldExtension } from '../spi/interfaces'
import type { CreateTransferInput, AuditTransferInput, ReverseTransferInput } from 'moyan-mfw-extension-ledger/shared'
...
export class LedgerTransferService {
   ...
   */
  async createTransfer(input: CreateTransferInput, maker?: { id?: string; text?: string }) {
    // bizType 注册制校验
  ...
  /** 审核（通过入队 / 驳回解冻） */
  async audit(input: AuditTransferInput, auditor?: { id?: string; text?: string }) {
  ...
  async reverse(input: ReverseTransferInput, maker?: { id?: string; text?: string }) {
  ...
  async repost(transferNo: string) {
  ...
  /** 批量重推（≤1000） */
  async batchRepost(transferNos: string[]) {
  ...
  async cancel(transferNo: string, operator?: { id?: string; text?: string }) {
  ...
  /** 查交易单 */
  async getTransfer(transferNo: string) {
    return this.storage.getTransfer(transferNo)
  }

  /** 按幂等键查 */
  async findByBizRef(bizRef: string, bizType: string) {
    return this.storage.findTransferByBizRef(bizRef, bizType)
  }

  /** 交易单分页 */
  async queryTransfers(filter: any) {
    return this.storage.queryTransfers(filter)
  }

  /** 流水分页 */
  async queryEntries(filter: any) {
