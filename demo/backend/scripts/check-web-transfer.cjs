...
(async () => {
  ...
  const [rows] = await conn.query("SELECT transferNo, bizRef, needReview, auditStatus, postStatus, holdType, amount, fromAccountId FROM ext_ledger_transfer WHERE bizRef = 'web-biz-1'");
